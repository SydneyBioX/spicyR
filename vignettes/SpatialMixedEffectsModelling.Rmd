...
`location()` returns a `DataFrame` object providing the location (`x` and `y`) 
and cell type (`cellType`) of each cell and the image it belongs to (`imageID`).

`phenotype()` returns a `DataFrame` object providing the corresponding subject 
...
## Testing for change in colocalisation between Responders and Non-Responders 
...
## Test for change in colocalisation between Responders and Non-Responders for 
