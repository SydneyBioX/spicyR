...
Here, we can observe that the most significant relationships occur between macrophages and dendritic cells/monocytes, suggesting that the two cell types are far more dispersed in compartmentalised tumours compared to cold tumours.

To examine a specific cell type-cell type relationship in more detail, we can use `spicyBoxplot()` and specify either `from = "Macrophages"` and `to = "DC_or_Mono"` or `rank = 1`.
