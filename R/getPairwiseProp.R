...
getPairwiseProp <- function(
  ...
  types <- as.character(getCellType(cells))
  ...
  res <- getPairwisePropCpp(
    as.numeric(cells$x[o]), as.numeric(cells$y[o]), match(types, allTypes)[o],
    c(0L, cumsum(tabulate(as.integer(img), nlevels(img)))), length(allTypes), k,
    match(m1, allTypes), match(m2, allTypes), includeZeroCells, as.integer(nThreads)
  )
