...
regionGrid <- function(pp, nbp=250){
  ...
  df <- as.data.frame(pp)
  k <- rep(NA,length(m))
  K <- knn(train=df[,c('x','y')], test=Grid[t(m)[1:length(m)],], cl=pp$region, k = 1)
...
regionPoly <- function(grid,region){
  rx <- range(grid$x)
  ry <- range(grid$y)
  mat <- matrix(grid$regions == region, nrow=length(unique(grid$x)), ncol=length(unique(grid$y)), byrow = TRUE)
  mat[is.na(mat)] <- FALSE
...
plotRegions <- function(pp, line.spacing = 21, xrange= c(0,1), yrange= c(0,1), nbp = 250, line.width=1){
...
cat('before hatchFun',.,'\n')
    hatchFun <- switch(.,"1" = hatchNull, "2" = hatch45,"3" = hatch315, "4" = hatch90, "5" = hatch180, "6" = hatchX, "7" = hatchPlus)
...
hatchingLines <- function(rPoly,allHatch, ordColumn, h90 = FALSE, xr, yr, rx, ry, line.width=1){
  ...
  intPoints <- purrr::map_dfr(rPoly$bdry,~{
    ...
    int <- purrr::map_dfr(seq_len(nrow(df)-1), ~{
      x1 = df[., ]
      x2 = df[. + 1, ]
      return(data.frame(t(line.intersection(x1, x2,hatch[1,], hatch[2,],  interior.only = TRUE))))
    })
  ...
  if(length(unlist(intPoints))>2){
    ...
    linesH <- purrr::map(pointSplit, ~{
      return(linesGrob(x = .$x/rx[2], y = .$y/ry[2], gp=gpar(col = 1, lwd=line.width)))
...
hatch45 <- function(rPoly, width, rx, ry, line.width=1){
  ...
  lines <- hatchingLines(rPoly, allHatch, ordColumn = 1, h90 = FALSE, xr, yr, rx, ry, line.width=line.width)
...
hatch180 <- function(rPoly, width, rx, ry, line.width=1){
  ...
  lines <- hatchingLines(rPoly, allHatch, ordColumn = 2, h90 = FALSE, xr, yr, rx, ry, line.width=line.width)
...
hatch90 <- function(rPoly, width, rx, ry, line.width=l){
  ...
  lines <- hatchingLines(rPoly, allHatch, ordColumn = 1, h90 = TRUE, xr, yr, rx, ry, line.width=line.width)
...
line.intersection <-function (P1, P2, P3, P4, interior.only = TRUE)
{
  ...
  if (interior.only) {
    ...
    lambda2 <- -((X - P3[1]) * dx2 + (Y - P3[2]) * dy2)/(dx2^2 +
                                                           dy2^2)
    if (!((lambda1 >= 0) & (lambda1 <= 1) & (lambda2 >= 0) &
          (lambda2 <= 1))) {
      return(c(NA, NA))
    }
  }
