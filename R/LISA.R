...
lisa <- function(cells, Rs = c(2, 20, 50, 100), window = "square", window.length = 20) {
    ...
    if (length(cells$cellID) != length(unique(cells$cellID))) 
        stop("The number of rows in cells does not equal the number of unique cellIDs")
  ...
  if (is.null(cells$imageID)) {
    ...
    generateCurves <- BiocParallel::bplapply(as.list(unique(cells$SampleID)), function(k, 
        cells, Rs, cellTypes, window, window.length) {
    ...
    }, cells = cells, Rs = Rs, cellTypes = unique(cells$cellType), window = window, 
...
localLCurves2 <- function(cells, Rs = c(1, 20, 50, 100), window = "square", window.length = 20) {
    ...
    generateCurves <- lapply(as.list(unique(cells$SampleID)), function(k, cells, 
        Rs, cellTypes, window, window.length) {
        use <- cells$SampleID == k
...
localLCurves2Inhom <- function(cells, Rs = c(1, 20, 50, 100), window = "square", 
    window.length = 20) {
