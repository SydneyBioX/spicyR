...
hatchingPlot <- function(data, image = NULL, window = "concave", line.spacing = 21, 
    ...
    if (!is(data, "SegmentedCellExperiment") | is.null(data$region)) 
        stop("Please provide a SegmentedCellExperiment with region information please.")
    ...
    if (!is.null(image)) {
        if (any(!image %in% rownames(data))) 
            stop("Some of the images are not in your SegmentedCellExperiment")
